...
pkgdatadir = $(datadir)/giracoin
pkgincludedir = $(includedir)/giracoin
pkglibdir = $(libdir)/giracoin
pkglibexecdir = $(libexecdir)/giracoin
...
am__aclocal_m4_deps = $(top_srcdir)/build-aux/m4/ax_boost_base.m4 \
	...
	$(top_srcdir)/build-aux/m4/giracoin_find_bdb48.m4 \
	$(top_srcdir)/build-aux/m4/giracoin_qt.m4 \
	$(top_srcdir)/build-aux/m4/giracoin_subdir_to_include.m4 \
...
CONFIG_HEADER = $(top_builddir)/src/config/giracoin-config.h
CONFIG_CLEAN_FILES = libgiracoinconsensus.pc share/setup.nsi \
	share/qt/Info.plist src/test/buildenv.py \
	qa/pull-tester/run-giracoind-for-test.sh \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libgiracoinconsensus.pc.in \
...
COPYRIGHT_HOLDERS = The %s Developers - Gira Financial Group AG (http://www.gira-financial.com)
...
GIRACOIN_DAEMON_NAME = giracoind
GIRACOIN_GUI_NAME = giracoin-qt
GIRACOIN_TX_NAME = giracoin-tx
GIT = /usr/bin/git
GREP = /bin/grep
...
OTOOL64 = 
PACKAGE = giracoin
PACKAGE_BUGREPORT = https://giracoin.com/
PACKAGE_NAME = Giracoin Core
PACKAGE_STRING = Giracoin Core 0.13.1
PACKAGE_TARNAME = giracoin
...
install_sh = ${SHELL} /root/giracoin/build-aux/install-sh
...
pkgconfig_DATA = libgiracoinconsensus.pc
GIRACOIND_BIN = $(top_builddir)/src/$(GIRACOIN_DAEMON_NAME)$(EXEEXT)
GIRACOIN_QT_BIN = $(top_builddir)/src/qt/$(GIRACOIN_GUI_NAME)$(EXEEXT)
GIRACOIN_CLI_BIN = $(top_builddir)/src/$(GIRACOIN_CLI_NAME)$(EXEEXT)
GIRACOIN_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
empty := 
space := $(empty) $(empty)
OSX_APP = Giracoin-Qt.app
...
OSX_INSTALLER_ICONS = $(top_srcdir)/src/qt/res/icons/giracoin.icns
...
WINDOWS_PACKAGING = $(top_srcdir)/share/pixmaps/giracoin.ico \
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_giracoin_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_giracoin_coverage.info test_giracoin.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/giracoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Giracoin-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
CLEANFILES = $(OSX_DMG) $(GIRACOIN_WIN_INSTALLER)
...
src/config/giracoin-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/giracoin-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/giracoin-config.h
$(top_srcdir)/src/config/giracoin-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/giracoin-config.h src/config/stamp-h1
libgiracoinconsensus.pc: $(top_builddir)/config.status $(srcdir)/libgiracoinconsensus.pc.in
...
src/test/buildenv.py: $(top_builddir)/config.status $(top_srcdir)/src/test/buildenv.py.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
qa/pull-tester/run-giracoind-for-test.sh: $(top_builddir)/config.status $(top_srcdir)/qa/pull-tester/run-giracoind-for-test.sh.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
...
$(GIRACOIN_WIN_INSTALLER): all-recursive
	$(MKDIR_P) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(GIRACOIND_BIN) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(GIRACOIN_QT_BIN) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(GIRACOIN_CLI_BIN) $(top_builddir)/release
...
$(OSX_APP)/Contents/Resources/giracoin.icns: $(OSX_INSTALLER_ICONS)
	$(MKDIR_P) $(@D)
	$(INSTALL_DATA) $< $@

$(OSX_APP)/Contents/MacOS/Giracoin-Qt: $(GIRACOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Giracoin-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Giracoin-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_giracoin.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_giracoin -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_giracoin_filtered.info: test_giracoin.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_giracoin_filtered.info
#	$(MKDIR_P) qa/tmp
#	-@TIMEOUT=15 qa/pull-tester/run-giracoind-for-test.sh $(JAVA) -jar $(JAVA_COMPARISON_TOOL) qa/tmp/compTool $(COMPARISON_TOOL_REORG_TESTS)
...
#rpc_test.info: test_giracoin_filtered.info
...
#test_giracoin_coverage.info: baseline_filtered_combined.info test_giracoin_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_giracoin_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_giracoin_filtered.info block_test_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_giracoin_filtered.info -a block_test_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_giracoin.coverage/.dirstamp:  test_giracoin_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
clean-local:
	rm -rf coverage_percent.txt test_giracoin.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
